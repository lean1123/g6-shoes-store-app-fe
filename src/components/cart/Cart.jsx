    ...
    <div className="flex justify-center p-6 bg-gray-100">
      ...
      <div className="w-2/3 bg-white p-6 shadow-md rounded-lg mr-6">
        <form action="/cart" method="post">
          ...
          <p className="mb-6">
            Bạn đang có <span className="font-semibold">{quantity} sản phẩm</span> trong giỏ hàng
          </p>
          ...
          <div className="flex items-center justify-between border p-4 rounded-md mb-6">
            ...
            <div className="flex-1 ml-4">
              <p className="font-medium">Giày Thể Thao Nam Biti’s Hunter X LiteDash - Original Edition 2K24 HSM007501XDL</p>
              <div className="flex justify-start items-center space-x-4 text-gray-500 mt-2">
                <p>Màu sắc: <span className="ml-2">○</span></p>
                <p>Kích thước: 39</p>
              </div>
              {/* Price per item */}
              <p className="font-semibold text-red-500 mt-2">Giá: {pricePerItem.toLocaleString()} ₫</p>
            </div>

            {/* Quantity and Total Price Section */}
            <div className="flex flex-col items-start mt-4 space-y-2">
              <div className="flex items-center space-x-2">
                <button type="button" onClick={decreaseQuantity} className="px-2 py-1 bg-gray-200 rounded">-</button>
                <span>{quantity}</span>
                <button type="button" onClick={increaseQuantity} className="px-2 py-1 bg-gray-200 rounded">+</button>
              </div>
      ...
      <div className="w-1/3 bg-white p-6 shadow-md rounded-lg">
        ...
        <div className="flex justify-between mb-4">
          <span>Tổng tiền:</span>
          <span className="text-red-500 font-semibold text-lg">{totalPrice.toLocaleString()} ₫</span>
        </div>
        <p className="text-gray-500 mb-6">Phí vận chuyển sẽ được tính ở trang thanh toán.</p>
        <p className="text-gray-500 mb-6">Bạn cũng có thể nhập mã giảm giá ở trang thanh toán.</p>
